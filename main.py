...
conn = sql.connect(host='localhost', user='root', password='1234',
                   database='sms', auth_plugin='mysql_native_password')
c1 = conn.cursor()
chpasswd = 'd'
print('''+--------------------------+
/////////////////////////                                      
...
while 5 > 1:
    ...
    if loggin == 1:
        while 3 > 1:
            ...
            c1.execute(
                'select product_name,cost_per_product from stock where product_no =' + b)
            data = c1.fetchall()
            data1 = list(data[0])
            ...
            if appr == 'y' or appr == 'Y':
                c1.execute(
                    "update stock set stock = stock-1  where product_no= " + b)
                c1.execute(
                    "update stock set purchased = purchased+1 where product_no=" + b)
                bill += int(data1[1])
                it += 1
...
*******PLEASE VIST AGAIN*******''')
        ...
        if ch == 1:
            ...
            prdno = input(
                "Enter the product number of the product for which the stock is going to be updated:")
            ...
            stock12 = input(
                'Enter the number of stocks of the new product arrived:')
            pch = '0'
